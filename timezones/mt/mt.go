/*
Package mt provides Mountain Time timezone support for meridian.

MT represents the America/Denver IANA timezone, which observes Mountain Time depending on the time of year.
...
Create MT times:
	...
	now := mt.Now()
	specific := mt.Date(2024, time.December, 25, 10, 30, 0, 0)
	parsed, _ := mt.Parse(time.RFC3339, "2024-12-25T10:30:00Z")

Convert to MT from other timezones:

	eastern := est.Now()
	pacific := mt.FromMoment(eastern)

Convert from standard time.Time:

	stdTime := time.Now()
	typedTime := mt.FromMoment(stdTime)

The mt.Time type is an alias for meridian.Time[mt.Timezone], providing
compile-time timezone safety. Functions that accept mt.Time can only receive
times explicitly typed as Mountain Time, preventing timezone confusion.
*/
package mt
...
var location = mustLoadLocation("America/Denver")
...
// Timezone represents the Mountain Time timezone.
...
// FromMoment converts any Moment to MT time.
func FromMoment(m meridian.Moment) Time {
	return meridian.FromMoment[Timezone](m)
}

// Parse parses a formatted string and returns the time value it represents in MT.
// The layout defines the format by showing how the reference time would be displayed.
// The time is parsed in the America/Denver location.
func Parse(layout, value string) (Time, error) {
	return meridian.Parse[Timezone](layout, value)
}

// Unix returns the MT time corresponding to the given Unix time,
// sec seconds and nsec nanoseconds since January 1, 1970 UTC.
func Unix(sec, nsec int64) Time {
	return meridian.Unix[Timezone](sec, nsec)
}

// UnixMilli returns the MT time corresponding to the given Unix time,
// msec milliseconds since January 1, 1970 UTC.
func UnixMilli(msec int64) Time {
	return meridian.UnixMilli[Timezone](msec)
}

// UnixMicro returns the MT time corresponding to the given Unix time,
