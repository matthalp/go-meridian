...
JST represents the Asia/Tokyo IANA timezone, which observes Japan Standard Time depending on the time of year.
...
Create JST times:
	...
	now := jst.Now()
	specific := jst.Date(2024, time.December, 25, 10, 30, 0, 0)
	parsed, _ := jst.Parse(time.RFC3339, "2024-12-25T10:30:00Z")

Convert to JST from other timezones:

	eastern := est.Now()
	pacific := jst.FromMoment(eastern)

Convert from standard time.Time:

	stdTime := time.Now()
	typedTime := jst.FromMoment(stdTime)

The jst.Time type is an alias for meridian.Time[jst.Timezone], providing
compile-time timezone safety. Functions that accept jst.Time can only receive
times explicitly typed as Japan Standard Time, preventing timezone confusion.
*/
package jst
...
// The time is parsed in the Asia/Tokyo location.
