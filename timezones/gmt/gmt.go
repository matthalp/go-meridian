/*
Package gmt provides Greenwich Mean Time timezone support for meridian.

GMT represents the Europe/London IANA timezone, which observes Greenwich Mean Time depending on the time of year.
...
Create GMT times:
	...
	now := gmt.Now()
	specific := gmt.Date(2024, time.December, 25, 10, 30, 0, 0)
	parsed, _ := gmt.Parse(time.RFC3339, "2024-12-25T10:30:00Z")

Convert to GMT from other timezones:

	eastern := est.Now()
	pacific := gmt.FromMoment(eastern)

Convert from standard time.Time:

	stdTime := time.Now()
	typedTime := gmt.FromMoment(stdTime)

The gmt.Time type is an alias for meridian.Time[gmt.Timezone], providing
compile-time timezone safety. Functions that accept gmt.Time can only receive
times explicitly typed as Greenwich Mean Time, preventing timezone confusion.
*/
package gmt
...
var location = mustLoadLocation("Europe/London")
...
// Timezone represents the Greenwich Mean Time timezone.
...
// FromMoment converts any Moment to GMT time.
func FromMoment(m meridian.Moment) Time {
	return meridian.FromMoment[Timezone](m)
}

// Parse parses a formatted string and returns the time value it represents in GMT.
// The layout defines the format by showing how the reference time would be displayed.
// The time is parsed in the Europe/London location.
func Parse(layout, value string) (Time, error) {
	return meridian.Parse[Timezone](layout, value)
}

// Unix returns the GMT time corresponding to the given Unix time,
// sec seconds and nsec nanoseconds since January 1, 1970 UTC.
func Unix(sec, nsec int64) Time {
	return meridian.Unix[Timezone](sec, nsec)
}

// UnixMilli returns the GMT time corresponding to the given Unix time,
// msec milliseconds since January 1, 1970 UTC.
func UnixMilli(msec int64) Time {
	return meridian.UnixMilli[Timezone](msec)
}

// UnixMicro returns the GMT time corresponding to the given Unix time,
